...
export const NextKeyChip = memo(function NextKeyChip({
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 sm:gap-3">
      <span className="text-[8.5px] sm:text-[9.5px] font-bold uppercase tracking-wider text-[var(--ink-soft)]">
        {t("game.nextKey")}
      </span>

      <div className="pixel-panel flex items-center gap-2 rounded-md px-2.5 py-1 sm:px-3 sm:py-1.5">
        <span className="font-khmer text-xl sm:text-2xl leading-none" aria-hidden="true">
          {displayGlyph(target)}
        </span>
        {hit ? (
          <kbd className="rounded border-2 border-[var(--panel-edge)] bg-[var(--key-face)] px-1.5 py-0.5 font-retro text-[9.5px] sm:text-[10px] uppercase text-[var(--key-ink)]">
            {hit.label === " " ? t("keyboard.space") : hit.label}
          </kbd>
        ) : null}
        {/* Modifiers first, in the order they are pressed. */}
        {hit?.alt ? (
          <span className="font-retro text-[8.5px] sm:text-[9.5px] text-[var(--coin)]">
            {isMac ? t("game.useOption") : t("game.useAlt")}
          </span>
        ) : null}
        {hit?.shift ? (
          <span className="font-retro text-[8.5px] sm:text-[9.5px] text-[var(--coin)]">{t("game.useShift")}</span>
        ) : null}
      </div>

      {nameKey ? (
        <span className="text-[10px] sm:text-[11px] font-semibold text-[var(--ink-soft)]">{t(nameKey)}</span>
      ) : null}
