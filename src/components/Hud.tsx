...
function Stat({
  ...
  return (
    <div className="flex min-w-[4.5rem] flex-col gap-0.5">
      ...
      <span className={`font-retro text-[11px] leading-none sm:text-[13px] ${toneClass}`}>
        {value}
      </span>
    </div>
  );
...
export const Hud = memo(function Hud({ engine }: { engine: TypingEngine }) {
  ...
  return (
    <div className="pixel-panel flex flex-col gap-3 rounded-md p-3">
      <div className="flex flex-wrap items-start gap-x-5 gap-y-3">
        ...
        <div className="flex min-w-[4.5rem] flex-col gap-0.5">
          ...
          <div className="flex gap-0.5 text-[13px] leading-none" aria-label={`${engine.lives}`}>
      ...
      <div className="flex items-center gap-2">
        ...
        <div
          className="h-3 flex-1 overflow-hidden rounded-sm border-2 border-[var(--panel-edge)] bg-[var(--key-face)]"
          role="progressbar"
          aria-valuemin={0}
          aria-valuemax={100}
        ...
        <span className="font-retro text-[10px] text-[var(--ink-soft)]">
